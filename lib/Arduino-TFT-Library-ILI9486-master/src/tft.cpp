...
void TFT::init() {
  ...
  //      writeCommand(0x20); // Display Inversion OFF   RPi LCD (A)
       ...
       */
       ...
       /* Serial.println(readcmd(0x0A));
              wynik= SPI.transfer(0x00);
...
void TFT::startWrite(void){
    SPI.beginTransaction(SPISettings(_freq, MSBFIRST, SPI_MODE3));
    //TFT_CS_LOW();
